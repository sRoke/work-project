angular.module('qh-assistance-wap-front').factory('wxService', ['$http', '$q', "$timeout", 'errorService', 'appConfig', 'userService', 'alertService', "$state", '$cookies',
    function ($http, $q, $timeout, errorService, appConfig, userService, alertService, $state, $cookies) {
        /**
         * 初始化微信
         */
        function init() {
        ...
        function wxLogin(urls, loginUrl) {
            ...
            if (ua.match(/MicroMessenger/i) && !ua.match(/windows/i)) {
                $http
                    .get(appConfig.apiPath + '/weiXin/wxOauthLogin?backUrl=' + url + "&url=" + loginUrl)
                ...
                window.Wechat.isInstalled(function (installed) {
                    ...
                    $http({
                        ...
                        window.Wechat.auth(scope, genState.state, function (response) {
                            ...
                            // 进行微信登陆
                            $http.get(appConfig.apiPath + "/weiXin/wxLoginVerify?code=" + response.code + "&state=" + response.state + "&wxType=qhApp&url=" + loginUrl)
                                .then(function (resp) {
                ...
                $http
                    .get(appConfig.apiPath + '/weiXin/wxWebLogin?backUrl=' + url + "&url=" + loginUrl)
                    .success(function (data) {
                        window.location.href = data.uri;
                    });
            }
        ...
        function getUserInfo(userId) {
            ...
            $http({
                method: "GET",
                url: appConfig.apiPath + '/user/userInfo',
                params: {
                    userId: userId,
                }
