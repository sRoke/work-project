(function () {
    ...
    function lotteryController($scope, appConfig, $http, $location, $state, $mdDialog, $templateCache, wxService) {
        ...
        if ($location.search().id && $location.search().phone) {
            ...
            $scope.data = {};
            // $scope.login = true;
            // $scope.data.img = 'https://img.kingsilk.net/d22145ce7f565673e8e5515bde347203';
            //转盘实例 //.img1为需要旋转元素的id;
            $scope.lottery = {};
            //默认每次进页面可以转5次
            // setTimeout(function () {
            ...
            $scope.pageChange = function () {
                $http({
                    ...
                    params: {
                        // 'id': $location.search().id ? $location.search().id : 'qh1200057395'
                        'id': $location.search().id,
                    ...
                    $scope.turn = function (ev) {
                        ...
                        $http({
                            ...
                            $scope.clickBtn = true;
                            $scope.dialogNoNumber("您来晚了,奖品已抽完!");
                        });
                    };
