...
function handleRespErr(response) {
    ...
    if (response.status == '401' || response.data.status == '401') {  //未推出登录,重新拉取授权
        jso.wipeTokens();
        ...
        if(response.data.status == 11002 || response.data.status == 11003 ||response.data.status == 11004 ||response.data.status == 11005){
            loginService.getCheck();
            stateService.go('main.brandApp.joinUs');
        }else  if(response.data.status == 11006){  //推出登录,重新登录
            jso.wipeTokens();
            store.set(conf.brandAppId, '');
            store.set(conf.token, '');
            $http({
