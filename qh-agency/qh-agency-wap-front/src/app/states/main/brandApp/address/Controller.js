...
class Controller {
    constructor(_$scope, _$stateParams, _$httpParamSerializer, _alertService, _$http, _$timeout, _loginService, _$state, _$location) {
        ...
        $scope.setDefault = function (addressList, index) {
            ...
            $http({
                method: "PUT",               ///brandApp/{brandAppId}/partner/{partnerId}/order/{id}/updateAddr  PUT          /{id}/chooseAddr
                url: conf.apiPath + "/brandApp/" + $scope.brandAppId + "/partner/111111111/order/" + $scope.orderId + "/changeAddr",
                params: {
                    addrId: addressList.id
                },
                headers: {
                    'Authorization': 'Bearer ' + loginService.getAccessToken(),
                    "brandApp-Id": $scope.brandAppId
