...
class Controller {
    constructor(_$scope,
        ...
        vm.save = function () {
            ...
            $http({
                method: "POST",
                url: conf.apiPath + "/brandApp/" + $scope.brandAppId + "/shop/" + $scope.storeId + "/addr",
                headers: {
               ...
               if(resp.data.status=='200'){
                   $state.go("main.brandApp.store.personalCenter.address", {orderId: $scope.orderId,from:$scope.from}, {reload: true});
               }
            }, function (resp) {
                //error
            });
