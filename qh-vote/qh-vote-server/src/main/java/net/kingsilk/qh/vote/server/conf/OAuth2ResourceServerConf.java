...
import net.kingsilk.qh.vote.service.service.StaffUserDetailsService;
...
public class OAuth2ResourceServerConf {
    ...
    ResourceServerTokenServices resourceServerTokenServices(
        ...
        ts.setClientId(props.getQhVote().getServer().getClientId());
        ts.setClientSecret(props.getQhVote().getServer().getClientSecret());
...
//    }
    ...
    @Bean
    public VoteAppIdFilter brandAppIdFilter() {

        return new VoteAppIdFilter();
    }

    @Bean
    ResourceServerConfigurerAdapter resourceServerConfigurerAdapter(
            ResourceServerTokenServices resourceServerTokenServices,
            VoteAppIdFilter voteAppIdFilter
    ) {
        return new ResourceServerConfigurerAdapter() {
            ...
            public void configure(ResourceServerSecurityConfigurer resources) {
                ...
                resources.resourceId("RSC_ID_qh-vote-server")
...
//                 处理 "Company-Id" http 请求头
                http.addFilterBefore(voteAppIdFilter, X509AuthenticationFilter.class);
