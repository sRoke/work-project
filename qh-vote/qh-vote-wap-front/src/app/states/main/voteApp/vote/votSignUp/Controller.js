                ...
                $mdDialog.show({
                    ...
                    controller: ['$mdDialog', function ($mdDialog) {
                        ...
                        vmd.cancel = function () {
                            ...
                            $state.go('main.voteApp.vote.votHome');
            ...
            $http({
                ...
                $http({
                    ...
                    if(resp.data.status == 10024) {
                        ...
                        alertService.msgAlert("exclamation-circle", resp.data.data.msg);
                        $scope.fallBack('main.voteApp.vote.votHome');
                        $scope.clickSave = false;
                        return;
                    }else if(resp.data.status == 200) {
                        if(resp.data.data.msg == 'check'){
                            alertService.msgAlert("exclamation-circle", '报名成功!马上为您审核!').then(function () {
                                $scope.fallBack('main.voteApp.vote.myVote',{id:resp.data.data.voteWorkId});
                                $scope.clickSave = false;
                            });
                        }else if(resp.data.data.msg == 'unCheck'){
                            alertService.msgAlert("exclamation-circle", '报名成功!').then(function () {
                                $scope.fallBack('main.voteApp.vote.myVote',{id:resp.data.data.voteWorkId});
                                $scope.clickSave = false;
                            });
                        }
